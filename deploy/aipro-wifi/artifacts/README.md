...
| 文件 | md5 | 说明 |
|---|---|---|
| aic8800_fdrv.ko | 2396b6f4... | **当前生产版**（AICDBG ~1.4条/s 仅 dmesg RAM，无害）|
| aic_load_fw.ko | c6aa6248... | 配套 |
| aic8800_fdrv.clean.ko | b04507dda3e7... | 清桩版（AICDBG=0）。**注意：新旧版均有 AP 偶发断连**（客户端断开→"key not allowed"→beacon 短暂失败），与驱动版本无关，疑与 bond-lan 网络拓扑或 USB 时序相关，待查 |
| aic_load_fw.clean.ko | ecb081cac49e... | 配套 clean 版（fdrv+load_fw 必须成对替换，aicwf_rxbuff_* 符号依赖）|
