...
#if 1
	//waiting for rx/tx thread init finish (timeout-bounded, was infinite loop)
	{
		int wait_cnt = 0;
		for (wait_cnt = 0; wait_cnt < 50; wait_cnt++) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 14, 0)
			if (bus_if->busrx_thread->__state == TASK_INTERRUPTIBLE &&
				bus_if->bustx_thread->__state == TASK_INTERRUPTIBLE)
#else
			if (bus_if->busrx_thread->state == TASK_INTERRUPTIBLE &&
				bus_if->bustx_thread->state == TASK_INTERRUPTIBLE)
#endif
				break;
			AICWFDBG(LOGINFO, "%s waiting rx/tx init (%d)\r\n", __func__, wait_cnt);
			msleep(100);
		}
		if (wait_cnt >= 50)
			printk(KERN_ERR "AIC8800: rx/tx thread init TIMEOUT, continue anyway\n");
	}
#endif
