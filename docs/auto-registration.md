...
| **新机器 client** | aipro/mac/ningsure/lwpc 等 | 运行 `register.sh`，从池中取 peer，安装 sing-box |
...
```
 ...
 │    7. 提示用户: 去 lwtop 跑 server.sh --activate ningsure       │
 └─────────────────────────────────────────────────────────────────┘
```
...
### Phase 1: Server 预生成 (`server.sh --pre-gen <N>`)
...
### Phase 2: Client 注册 (`register.sh <client_id>`)

```
[新机器上运行]
bash register.sh ningsure

  1. 检测平台 + 架构 (复用 client.sh 逻辑)
...
### Phase 3: Server 激活 (`server.sh --activate <client_id>`)
...
状态流转:
```
available  ──register.sh 取用──▶  used  ──server.sh --activate──▶  activated
```
...
| peer 重用（同一配置多机使用） | IP 冲突 + 密钥泄露 | register.sh 原子标记 `status: used` 并 push 回 gitee |
| server 公钥泄露 | 无风险（公钥本就公开） | 公钥写在 readme.md，任何人可见 |
| gitee token 泄露 | 所有 peer 私钥泄露 | token 权限最小化（只读该仓库）；定期轮换 |
| man-in-the-middle 注册过程 | client 拿到错误的 server 公钥 | SERVER_PUBKEY 硬编码在 register.sh 中作为校验值 |

### 安全原则

1. **私钥始终在 gitee 私有仓库**：register.sh 拉取后写入本地，不从公开渠道传输
...
5. **激活分离**：register.sh 只完成 client 侧配置；server 侧激活需要管理员手动操作（防止恶意注册直接上线）
...
├── peers/                         # 预生成 peer 池 (gitee 私有, gitignore 中的例外)
│   └── slot-01.json               # peer JSON 示例
...
sudo bash server.sh --pre-gen 5 --push
# 生成并推送到 gitee
```
